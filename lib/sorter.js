...
function isortRequest (requestType, pythonEnv, useEntireEditor, textEditor) {
  ...
  const spawnOptions = { env: pythonEnv, input: stdinInput }

  // try python3 first
  let pyResponse = cp.spawnSync('python3', pythonArgs, spawnOptions)
  if (pyResponse.error != null) {
    if (pyResponse.error.code === 'ENOENT') {
      // fallback to python
      pyResponse = cp.spawnSync('python', pythonArgs, spawnOptions)
      if (pyResponse.error != null) {
        if (pyResponse.error.code === 'ENOENT') {
...
function handleIsortResponse (response, insertType, textEditor) {
  if (response.type === 'error') {
    error(`atom-isort: could not sort imports: ${response.description}`)
  }
  ...
  if (response.type === 'sort_text_response_no_change') {
    return true
  }
...
const info = (msg, detail = null) => notification('info', msg, detail)
const warning = (msg, detail = null) => notification('warning', msg, detail)
const error = (msg, detail = null) => notification('error', msg, detail)
